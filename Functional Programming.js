...
true. In other words, it filters the array, based on the function passed to it. Like map, it does this without needing to modify the 
original array.

The callback function accepts three arguments. The first argument is the current element being processed. The second is the index of 
that element and the third is the array upon which the filter method was called.
...
Program below uses a combination of filter and map on watchList to assign a new array of objects with only title and rating keys. 
The new array should only include objects where imdbRating is greater than or equal to 8.0. 
Note: rating values are saved as strings in the object and will need to be converted into numbers to perform mathematical operations on 
them.
...
/*
Steps to solve: First, Array.prototype.filter is used to filter the array so it’s left with elements that have imdbRating > 8.0.
